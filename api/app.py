...
@app.route(f'{API_PREFIX}/validate-road-hazards', methods=['POST'])
@require_api_key
def validate_road_hazards():
    """
    Validate if images contain legitimate road hazards using Gemini API
    ...
    try:
        ...
        for i, file in enumerate(files):
            ...
            try:
                ...
                # Prepare comprehensive road hazard validation prompt
                hazard_prompt = """
                Analyze this image carefully for ANY road-related hazards, damage, or safety concerns. Look for:

                ROAD SURFACE DAMAGE:
                1. POTHOLES: Holes, depressions, cavities, or craters in road surface
                2. ROAD CRACKS: Fissures, splits, cracks, or fractures in asphalt/concrete
                3. DAMAGED PAVEMENT: Broken, crumbling, deteriorated, or missing road surface
                4. UNEVEN SURFACES: Bumps, dips, humps, or irregular road surfaces
                5. EROSION DAMAGE: Washed out roads, undermined pavement, soil erosion

                ROAD HAZARDS & OBSTACLES:
                6. DEBRIS ON ROADS: Objects, materials, garbage, or obstacles blocking traffic
                7. ACCIDENT DAMAGE: Vehicle accidents, collision damage, crash scenes, wreckage
                8. FALLEN OBJECTS: Trees, rocks, branches, construction materials on roadway
                9. FLOODING/WATER HAZARDS: Standing water, flooding, deep puddles, water covering roads, submerged roads, flood damage
                10. CONSTRUCTION HAZARDS: Road work, barriers, equipment, unsafe construction zones
                11. ABANDONED VEHICLES: Broken down cars, trucks blocking traffic lanes

                TRAFFIC & SAFETY HAZARDS:
                12. TRAFFIC CONGESTION: Heavy traffic jams, gridlock, blocked intersections
                13. EMERGENCY SITUATIONS: Fire trucks, ambulances, police activity on roads
                14. DANGEROUS DRIVING: Reckless driving, vehicles in wrong lanes, near misses
                15. PEDESTRIAN HAZARDS: People walking on highways, unsafe crossings
                16. VISIBILITY ISSUES: Fog, smoke, dust storms affecting road visibility
                17. WEATHER HAZARDS: Severe storms, hail, tornado damage on roads

                FIRE & EXPLOSION HAZARDS:
                18. VEHICLE FIRES: Cars, trucks, motorcycles on fire on roadways
                19. ROADSIDE FIRES: Grass fires, brush fires near or affecting roads
                20. STRUCTURAL FIRES: Buildings, bridges, overpasses on fire near roads
                21. FUEL SPILLS: Gasoline, oil spills creating fire hazards on roads
                22. EXPLOSION DAMAGE: Blast damage to roads, vehicles, or infrastructure
                23. SMOKE HAZARDS: Heavy smoke reducing visibility and air quality on roads

                INFRASTRUCTURE ISSUES:
                24. DAMAGED SIGNS: Broken, fallen, or missing traffic signs and signals
                25. GUARDRAIL DAMAGE: Broken, damaged, or missing guardrails and barriers
                26. BRIDGE/OVERPASS DAMAGE: Structural damage to road infrastructure
                27. LIGHTING ISSUES: Broken streetlights, dark dangerous intersections
                28. DRAINAGE PROBLEMS: Clogged drains, poor water management on roads

                ACCEPTANCE CRITERIA:
                - Accept ANY image showing roads, streets, highways, parking lots, or traffic areas
                - Accept driveways, gas stations if they show road-related hazards
                - Accept accident scenes, emergency situations, traffic problems
                - Accept weather-related road hazards and visibility issues
                - Reject only: indoor spaces, buildings interior, non-transportation areas

                CRITICAL: If you see ANY of the above hazards, damage, or safety concerns in a road/traffic context, respond HAZARD_DETECTED.
                Be generous in detection - prioritize public safety over strict criteria.

                SPECIAL ATTENTION TO FLOODING:
                - ANY standing water on roads = HAZARD_DETECTED
                - Deep puddles covering road surface = HAZARD_DETECTED
                - Flooded or submerged roads = HAZARD_DETECTED
                - Water that could affect vehicle safety = HAZARD_DETECTED

                Respond with EXACTLY this format (no extra text):
                RESULT: HAZARD_DETECTED
                CONFIDENCE: 8
                REASONING: Standing water/flooding detected on road surface

                OR if no hazards:
                RESULT: NO_HAZARD
                CONFIDENCE: 8
                REASONING: Clean road with no visible hazards
                """

                # Send to Gemini for analysis
                response = gemini_model.generate_content([hazard_prompt, image])
                gemini_text = response.text.strip()

                # Debug logging
                print(f"Gemini response for image {i + 1}:")
                print(f"Raw response: {gemini_text}")
                print("---")

                # Parse Gemini response with improved logic
                ...
                try:
                    ...
                    # If no structured result found, use enhanced keyword detection
                    if not any(line.strip().startswith('RESULT:') for line in lines):
                        text_lower = gemini_text.lower()

                        # Enhanced flooding detection keywords
                        flooding_keywords = ['flooding', 'flood', 'water', 'standing water', 'submerged', 'puddle', 'water covering']
                        other_hazards = ['hazard', 'damage', 'dangerous', 'unsafe', 'blocked', 'accident', 'emergency']

                        all_keywords = flooding_keywords + other_hazards
                        found_keywords = [kw for kw in all_keywords if kw in text_lower]

                        if found_keywords:
                            hazard_detected = True
                            confidence = 7
                            reasoning = f"Detected: {', '.join(found_keywords[:2])}"
                        else:
                            confidence = 3
                            reasoning = "No hazard indicators found"

                except Exception as parse_error:
                    print(f"Error parsing Gemini response: {parse_error}")
                    # Simple fallback
                    text_lower = gemini_text.lower()
                    if any(word in text_lower for word in ['flooding', 'flood', 'water', 'hazard', 'damage', 'dangerous']):
                        hazard_detected = True
                        confidence = 5
                        reasoning = "Fallback detection found hazard indicators"
                    else:
                        confidence = 3
                ...
                if hazard_detected:
                    overall_hazard_detected = True

                print(f"Gemini validation for image {i + 1}: {'HAZARD' if hazard_detected else 'NO_HAZARD'} (confidence: {confidence})")
        ...
        hazard_count = sum(1 for result in validation_results if result['hazard_detected'])

        return jsonify(create_api_response(
            success=True,
            data={
                'overall_hazard_detected': overall_hazard_detected,
                'hazard_count': hazard_count,
                'total_images': len(validation_results),
                'validation_results': validation_results,
                'message': 'Road hazards detected' if overall_hazard_detected else 'No road hazards detected'
            },
            message=f'Validation completed. {"Road hazards detected" if overall_hazard_detected else "No road hazards found"} in {len(validation_results)} image(s).'
        ))

    except Exception as e:
        print(f"Error in road hazard validation: {e}")
        traceback.print_exc()
        return jsonify(create_api_response(
            success=False,
...
@app.route(f'{API_PREFIX}/test-gemini', methods=['POST'])
@require_api_key
def test_gemini():
    """
    Test endpoint to manually test Gemini responses
    """
    try:
        if not gemini_model:
            return jsonify(create_api_response(
        ...
        1. Is this a road or traffic-related scene?
        2. Do you see any hazards, damage, accidents, or safety concerns?
        3. What specifically do you observe?

        Be detailed in your response.
        """

        # Send to Gemini
        response = gemini_model.generate_content([test_prompt, image])
        gemini_text = response.text.strip()
