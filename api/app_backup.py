...
# Initialize OpenRouter AI with enhanced debugging
openrouter_enabled = False
openrouter_client = None

# Enhanced debugging for Railway deployment
print("🔍 OpenRouter API Initialization Debug:")
...
if config['OPENROUTER_API_KEY']:
    print(f"   API Key Length: {len(config['OPENROUTER_API_KEY'])}")
    print(f"   API Key Prefix: {config['OPENROUTER_API_KEY'][:10]}...")
...
if config['OPENROUTER_API_KEY']:
    try:
        ...
        if test_response and test_response.choices and test_response.choices[0].message.content:
            print(f"✅ OpenRouter API test successful: {test_response.choices[0].message.content.strip()}")
        else:
            print("⚠️ OpenRouter API test returned empty response")

        openrouter_enabled = True
        print("✅ OpenRouter AI initialized successfully")

    except Exception as e:
        print(f"❌ OpenRouter AI initialization failed: {e}")
        ...
        elif "QUOTA" in str(e).upper() or "LIMIT" in str(e).upper() or "429" in str(e):
    ...
    print("⚠️ OpenRouter API key not provided - AI descriptions disabled")
...
for key, value in config.items():
    if key in ['EMAIL_PASSWORD', 'MAPBOX_ACCESS_TOKEN', 'OPENROUTER_API_KEY'] and value:
...
print(f"OpenRouter AI enabled: {openrouter_enabled}")
    ...
    return jsonify(create_api_response(
        ...
        data={
            ...
            'endpoints': [
                ...
                f'{API_PREFIX}/debug/openrouter',
                f'{API_PREFIX}/test-openrouter'
            ]
        },
        message='Pothole Detection API is running'
    ))

@app.route(f'{API_PREFIX}/debug/openrouter', methods=['GET'])
def debug_openrouter():
    """Debug endpoint for OpenRouter API status - Railway deployment diagnostics"""
    try:
        debug_info = {
            'environment': os.getenv('RAILWAY_ENVIRONMENT', 'local'),
            'openrouter_enabled': openrouter_enabled,
            'openrouter_client_available': openrouter_client is not None,
            'api_key_configured': bool(config.get('OPENROUTER_API_KEY')),
            'api_key_length': len(config.get('OPENROUTER_API_KEY', '')) if config.get('OPENROUTER_API_KEY') else 0,
        ...
        if openrouter_enabled and openrouter_client:
            try:
                test_response = openrouter_client.chat.completions.create(
                    model="openai/gpt-3.5-turbo",
                ...
                debug_info['openrouter_test'] = {
                    'success': True,
                    'response': test_response.choices[0].message.content.strip() if test_response and test_response.choices else 'Empty response'
                }
            except Exception as e:
                debug_info['openrouter_test'] = {
                    'success': False,
            ...
            debug_info['openrouter_test'] = {
                'success': False,
                'error': 'OpenRouter not enabled or client not available'
            }

        return jsonify(create_api_response(
            success=True,
            data=debug_info,
            message='OpenRouter debug information'
        ))
...
def generate_description():
    ...
    try:
        ...
        print(f"   OpenRouter Client Available: {openrouter_client is not None}")

        if not openrouter_enabled:
            error_msg = 'OpenRouter AI is not available. Please check API key configuration.'
            print(f"❌ {error_msg}")
            return jsonify({
                'success': False,
                'error': error_msg,
                'debug_info': {
                    'openrouter_enabled': openrouter_enabled,
                    'api_key_present': bool(config.get('OPENROUTER_API_KEY')),
                    'environment': os.getenv('RAILWAY_ENVIRONMENT', 'local')
                }
            }), 503
        ...
        **Repair Priority:**
        ...
        # Generate description using OpenRouter with enhanced retry logic
        max_retries = 2
        retry_delay = 1  # seconds

        print(f"🤖 Starting OpenRouter description generation...")
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Create messages for vision models
                if "gpt-4o" in model_name:
                    ...
                    # For non-vision models, use text-only prompt
                    messages = [
                        {"role": "user", "content": "Generate a professional pothole/road damage report with these sections:\n\n**Damage Assessment:** [Brief description]\n**Severity Level:** [High/Medium/Low]\n**Safety Impact:** [Risk description]\n**Location Context:** [Road type description]\n**Repair Priority:** [Immediate/Urgent/Routine]\n\nProvide a realistic assessment for a typical road hazard."}
                    ]
                ...
                if response and response.choices and response.choices[0].message.content:
                    ...
                    print(f"✅ OpenRouter response successful (length: {len(response_text)})")
                    return jsonify({
                        'success': True,
                    ...
                    error_msg = "Empty response from OpenRouter"
                    print(f"⚠️ {error_msg}")
                    raise Exception(error_msg)

            except Exception as openrouter_error:
                error_message = str(openrouter_error).lower()

                # Handle specific OpenRouter errors
                if '503' in error_message or 'overloaded' in error_message or '502' in error_message:
                    if attempt < max_retries - 1:
                        print(f"OpenRouter overloaded, retrying in {retry_delay} seconds... (attempt {attempt + 1}/{max_retries})")
                        import time
                        ...
                        return jsonify({
                            'success': False,
                            'error': 'OpenRouter AI is currently overloaded. Please try again in a few minutes.',
                ...
                elif 'quota' in error_message or 'limit' in error_message or '429' in error_message:
                    return jsonify({
                        'success': False,
                    ...
                    # Generic error
                    if attempt < max_retries - 1:
                        print(f"OpenRouter error, retrying... (attempt {attempt + 1}/{max_retries}): {openrouter_error}")
                        import time
                        time.sleep(1)
                        continue
                    else:
                        return jsonify({
                            'success': False,
                            'error': f'AI description generation failed: {str(openrouter_error)}',
...
@app.route(f'{API_PREFIX}/test-openrouter', methods=['POST'])
@require_api_key
def test_openrouter():
    """
    Test endpoint to manually test OpenRouter responses
    """
    try:
        if not openrouter_client:
            return jsonify(create_api_response(
                success=False,
                error='OpenRouter AI service not available',
        ...
        openrouter_text = response.choices[0].message.content.strip()

        return jsonify(create_api_response(
            success=True,
            data={
                'openrouter_response': openrouter_text,
        ...
        print(f"Error in OpenRouter test: {e}")
        traceback.print_exc()
        return jsonify(create_api_response(
            success=False,
            error=f'OpenRouter test failed: {str(e)}',
            code='OPENROUTER_TEST_ERROR'
        )), 500
